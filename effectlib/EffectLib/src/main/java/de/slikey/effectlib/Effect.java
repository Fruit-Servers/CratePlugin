...
public abstract class Effect implements Runnable, Cloneable {
    ...
    // Reflective copy used by crate animations and OriginalEffect.clone().
    @Override
    public Effect clone() {
        try {
            Constructor<? extends Effect> clonedConstructor = this.getClass().getConstructor(EffectManager.class);
            Effect effect = clonedConstructor.newInstance(this.effectManager);
            effect.type = this.type;
            effect.color = this.color;
            effect.colorList = this.colorList;
            effect.colors = this.colors;
            effect.toColor = this.toColor;
            effect.toColorList = this.toColorList;
            effect.toColors = this.toColors;
            effect.arrivalTime = this.arrivalTime;
            effect.speed = this.speed;
            effect.particleData = this.particleData;
            effect.delay = this.delay;
            effect.period = this.period;
            effect.iterations = this.iterations;
            effect.duration = this.duration;
            effect.probability = this.probability;
            effect.callback = this.callback;
            effect.visibleRange = this.visibleRange;
            effect.autoOrient = this.autoOrient;
            effect.offset = this.offset;
            effect.relativeOffset = this.relativeOffset;
            effect.targetOffset = this.targetOffset;
            effect.yawOffset = this.yawOffset;
            effect.pitchOffset = this.pitchOffset;
            effect.yaw = this.yaw;
            effect.targetPlayers = this.targetPlayers;
            effect.material = this.material;
            effect.materialData = this.materialData;
            effect.particleCount = this.particleCount;
            effect.particleOffsetX = this.particleOffsetX;
            effect.particleOffsetY = this.particleOffsetY;
            effect.particleOffsetZ = this.particleOffsetZ;
            effect.particleSize = this.particleSize;
            effect.origin = this.origin;
            effect.target = this.target;
            effect.maxIterations = this.maxIterations;
            return effect;
